...
cv::Mat SuperpixelHierarchy::buildHierarchy(const cv::Mat &pImage, const cv::Mat &pContours, int pNbSpx, bool buildScribbleLevels, RangeImage &rangeImage, bool metrics[4])
{
    connect = 10;
    w = rangeImage.getWidth();
    h = rangeImage.getHeight();
    currentLevel = pNbSpx;
    img = pImage;
    contours = pContours;
    ...
    SH.buildTree(rangeImage, metrics, edge_shift);
    ...
    //Save output
    clustersImage = cv::Mat(h, w, CV_8UC3);
    for (int i = 0; i < w; i++)
    {
        for (int j = 0; j < h; j++)
        {
